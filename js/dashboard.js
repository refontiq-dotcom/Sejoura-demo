...
(function () {
  ...
  document.querySelectorAll('.btn-new-booking').forEach(btn => btn.addEventListener('click', showModal));
  ...
  document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      showToast('Filtre : ' + btn.textContent.trim().split('(')[0].trim());
    });
  });
  ...
  document.querySelectorAll('.settings-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      document.querySelectorAll('.settings-tab').forEach(t => {
        t.classList.remove('active', 'bg-brand-50', 'dark:bg-brand-950/40', 'text-brand-700', 'dark:text-brand-300');
        t.classList.add('text-slate-600', 'dark:text-slate-300');
      });
      tab.classList.add('active', 'bg-brand-50', 'dark:bg-brand-950/40', 'text-brand-700', 'dark:text-brand-300');
  ...
  if (occCtx) {
    new Chart(occCtx, {
      ...
      data: {
        ...
        datasets: [{ data: [42, 6, 4], backgroundColor: ['#10b981', '#0c8ce9', '#f59e0b'], borderWidth: 0, hoverOffset: 4 }]
      },
  ...
  if (revCtx) {
    new Chart(revCtx, {
      ...
      data: {
        ...
        datasets: [{ label: 'Recettes', data: [1800000, 2100000, 1950000, 2400000, 2650000, 2450000, 2200000], backgroundColor: '#0c8ce9', borderRadius: 8, borderSkipped: false }]
      },
      options: {
        responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } },
        scales: {
          y: { beginAtZero: true, grid: { color: isDark() ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' }, ticks: { callback: (v) => (v / 1000000).toFixed(1) + 'M', color: isDark() ? '#94a3b8' : '#64748b', font: { size: 11 } } },
